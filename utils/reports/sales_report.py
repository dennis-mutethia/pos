...
class SalesReport():
    ...
    def fetch(self, from_date, to_date, page=0):
        ...
        with self.db.conn.cursor() as cursor:
            query = """
            SELECT be.item_name, pc.name AS category_name, s.selling_price, SUM(COALESCE(be.qty,0)) sold 
            ...
            GROUP BY be.item_name, pc.name, s.selling_price
            ORDER BY sold DESC
            """
            ...
            sales = []
            for datum in data:                       
                sales.append(Sale(datum[0], datum[1], datum[2], datum[3]))
    ...
    def __call__(self):
        ...
        sales = self.fetch(from_date, to_date, page) 
        ...
        return render_template('reports/sales-report.html', page_title='Reports > Sales', helper=Helper(),
                               sales=sales, grand_total=grand_total,
